...
socialData.forEach((iconData, i) => {
    loader.load(iconData.file, (gltf) => {
        ...
        socialGroup.add(obj);
    }, undefined, (err) => console.error("Error loading model:", iconData.file, err));
});
...
if (contactForm) {
    contactForm.addEventListener("submit", async (e) => {
        ...
        await fetch("/api/contact.js", { method:"POST", body:data });
        alert("Message sent!");
        e.target.reset();
    });
}
...
if (reviewForm) {
    reviewForm.addEventListener("submit", async (e) => {
        e.preventDefault();
        const data = new FormData(e.target);
        await fetch("/api/review.js", { method:"POST", body:data });
        alert("Review sent!");
        e.target.reset();
    });
}
